...
uint8_t Sensors_Init(I2C_HandleTypeDef *hi2c){
	...
	if(check!=0x68 && check != 0x70) return 1; // blad - brak mpu
	...
	// 2. Inicjalizacja BME280
	    HAL_I2C_Mem_Read(hi2c, BME280, 0xD0, I2C_MEMADD_SIZE_8BIT, &check, 1, 100);
	        if (check != 0x58 &&check != 0x60) return 2; // Błąd - brak BME
...
//Autokalibracja żyroskopu przy starcie
	        ...
	        gyro_bias_x=sum_gx/100.0f;
	        gyro_bias_y=sum_gy/100.0f;
	        gyro_bias_z=sum_gz/100.0f;

	        flaga=0;
	        Sensors_Read(hi2c, &g_sensors_data);
	      //  Sensors_Read(&hi2c, &g_sensors_data);
	    return 0; // sukces
}

static uint8_t mpu_dma_raw[14];
volatile uint8_t i2c_dma_in_progress = 0;

// Callback HAL wywoływany automatycznie po odebraniu 14 bajtów przez DMA2
void HAL_I2C_MemRxCpltCallback(I2C_HandleTypeDef *hi2c) {
    if (hi2c->Instance == I2C1) {
        // Konwersja surowych bajtów z bufora DMA
        ...
        g_sensors_data.accel_x = (float)raw_ax / 16384.0f;
        g_sensors_data.accel_y = (float)raw_ay / 16384.0f;
        g_sensors_data.accel_z = (float)raw_az / 16384.0f;
        ...
        i2c_dma_in_progress = 0; // Zwolnienie blokady
    }
}

// Funkcja wyzwalająca odczyt w tle (zastępuje Sensors_Read)
void Sensors_TriggerRead_DMA(I2C_HandleTypeDef *hi2c) {
    if (!i2c_dma_in_progress) {
        i2c_dma_in_progress = 1;
        // 0x68 << 1 = 0xD0 (adres MPU6050), 0x3B = ACCEL_XOUT_H
        HAL_I2C_Mem_Read_DMA(hi2c, (0x68 << 1), 0x3B, I2C_MEMADD_SIZE_8BIT, mpu_dma_raw, 14);
    }
}
...
uint8_t Sensors_Read(I2C_HandleTypeDef *hi2c, Sensors_Data_t *data) {
    ...
    data->gyro_x = ((float)raw_gx / 16.4f) - gyro_bias_x;
...
/*    if (i2c_state == I2C_IDLE) {
        i2c_state = I2C_READING_MPU;
        if (HAL_I2C_Mem_Read_DMA(hi2c, MPU6050, 0x3B, I2C_MEMADD_SIZE_8BIT, mpu_dma_raw, 14) != HAL_OK) {
            i2c_state = I2C_IDLE; // Wycofanie blokady w razie błędu startu
            return 1;
        }
        return 0;
    }
    return 2; // Magistrala zajęta
}*/
...
/*
    ...
    for (uint8_t addr = 1; addr < 128; addr++) {
        if (HAL_I2C_IsDeviceReady(&hi2c, (addr << 1), 1, 10) == HAL_OK) {
            found_devices++; // Postaw tu breakpoint - sprawdzisz pod jakim adresem odpowiada układ
        }
    }
*/
    ...
    for (int i = 0; i < 100; i++) {
        HAL_I2C_Mem_Read(hi2c, MPU6050, 0x43, I2C_MEMADD_SIZE_8BIT, raw, 6, 100);
        sum_gx += (int16_t)(raw[0] << 8 | raw[1]);
        sum_gy += (int16_t)(raw[2] << 8 | raw[3]);
        sum_gz += (int16_t)(raw[4] << 8 | raw[5]);
        HAL_Delay(5);
    }
    gyro_bias_x = (sum_gx / 100.0f) / 16.4f;
    gyro_bias_y = (sum_gy / 100.0f) / 16.4f;
    gyro_bias_z = (sum_gz / 100.0f) / 16.4f;
